                ...
                if msg:
                    ...
                    if(current_sock._verified == 1 and current_sock._order_number >= 5 and current_sock._action == "getconvo"
                        ...
                        if msg.encode('utf-8') is not b'':
                            if msg != "update" and msg != "getconvo" and current_sock._action != "getfile":
                                current_sock.post_message(msg.encode('utf-8'))
                            if current_sock._action != "getfile":
                                current_sock.get_conversation()
                        #Will add basename to message db. current_sock._action instead of msg should fix.

                    if (current_sock._verified == 1 and current_sock._order_number == 5 and current_sock._action == "getfile"):
                            current_sock._basename = msg
                            current_sock._f = open(current_sock._basename, 'wb')
                    if(current_sock._verified == 1 and current_sock._order_number >= 6 and current_sock._action == "getfile" and msg != "endoffile"):
